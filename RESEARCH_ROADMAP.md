...
| Julio 2026 | Fase 2 (reproducción de cálculos) ejecutada parcialmente con `Dashboard_HuellaSocial` (commit `6f652dd9`); `decision_log.md` creado | **H-01 confirmado en el dato fuente** (`HuellaSocial_Consolidado.xlsx`, hoja `Agregado Total`, fila 2025). Se descubrió que la memoria fue corregida a mano en Overleaf el 2026-07-28 (commit `81cd560`) con los valores exactos recomendados por H-01, pero **sin corregir el Excel fuente ni el dashboard**, que siguen mostrando la cifra errónea — nuevo hallazgo crítico H-10 (ver `docs/auditoria/02_reproduccion_calculos.md`). El mismo commit introdujo 3 cambios metodológicos (proxy de P1, deduplicación DAES, supuesto de remuneración sectorial) sin registro en decision log — ahora documentados retroactivamente en `docs/reports/decision_log.md`. Pendiente: solicitar a los autores que corrijan el Excel/dashboard y completen C-01, D-04, D-06 a D-08, E-01/02/03. |
| 2026-07-29 | Respuesta de los autores a `SOLICITUD_AUTORES.md` verificada; H-01/H-10 cerrados | Los autores corrigieron la causa raíz de H-10 (fórmula errónea en `HuellaSocial_Consolidado.xlsx`) y regeneraron el dashboard — **verificado de forma independiente, coincide exactamente**. También verificado D-06 (α = 0,3776 = suma de coeficientes técnicos directos de la actividad 94, MIP 2018 BCCh, archivo ahora en el repo). C-01/C-03 quedaron documentados como "no existen como script, son fórmulas de Excel transcritas a mano" — transparencia real, pero el riesgo de transcripción manual que originó H-01 sigue sin resolverse de raíz. **Nuevo hallazgo H-11**: los cambios de texto reportados para D-06/D-08/M-01–M-04 en la memoria aún no tienen commit en `huellasocial` — mismo patrón de brecha reportado-vs-versionado que produjo H-10. Pendiente: D-04, D-07, y sincronizar los cambios de texto al repositorio. Ver `docs/auditoria/02_reproduccion_calculos.md` §4 y `SOLICITUD_AUTORES.md` ronda 2. |
| Julio 2026 | Fase 3 (auditoría estadística) ejecutada — `docs/auditoria/03_auditoria_estadistica.md` | Re-verificados aritméticamente todos los Cuadros 5.1–5.9 contra el Excel corregido: **sin errores adicionales a H-01/H-10**. 4 hallazgos 🟡 nuevos: H-12 (supuesto de estabilidad temporal del 2,41 % patrimonial no declarado en la validación externa), H-13 (umbral de "concordancia" 0,01/0,03 pp definido tras observar los datos, no a priori), H-16 (eje secundario de la Figura 5.1/CMF.png truncado en 0,07 en vez de 0, exagera visualmente la caída del aporte al PIB), H-17 (sin análisis de sensibilidad de α pese a que los propios autores documentan heterogeneidad 0,23–0,72 entre entidades). 2 hallazgos 🟢 de pulido (N distinto entre cuadros de producción y financiera sin nota aclaratoria; conteos 35/36/38/42 de cooperativas no reconciliados en un solo lugar). Ninguno crítico — la memoria pasa con nota alta en correspondencia dato↔conclusión y honestidad interpretativa. |
| Julio 2026 | Fase 4 (evaluación según rúbrica UANDES) ejecutada — `docs/auditoria/04_evaluacion_rubrica.md` | Puntaje ponderado inicial 4,05/5 (pendiente confirmar con el guía la fórmula puntaje→nota antes de comunicar cualquier nota). Reemplaza la evaluación preliminar de Fase 0 (basada solo en el PDF), cuyos juicios cambiaron sustancialmente tras el cierre verificado de H-01/H-10. |
| 2026-07-30 | Corrección de criterio en Fase 4 (Indicador 4 "Desarrollo") a solicitud del guía | El criterio original exigía un pipeline de software consolidado (estándar de Ingeniería en Computación); corregido a exigir rigor de aplicación de la metodología de cuentas satélite (estándar propio de Ingeniería Civil Industrial). Indicador 4 sube de 3,0 a 4,0/5 con debilidades metodológicas específicas (α sin calibración empírica pese a datos disponibles; panel DAES tratado por exclusión sin justificar alternativas). Indicador 3 sube de 4,0 a 4,9 (el desajuste de período 2014-2024 vs. 2013-2025 no penaliza porque el trabajo excede lo comprometido). Indicador 5 sube de 4,0 a 4,5 (H-12/H-13 son de alcance acotado). **Puntaje ponderado final: 4,54/5.** Registrado en `docs/reports/decision_log.md`. |

| Julio 2026 | Fase 5 (retroalimentación accionable) ejecutada — `docs/auditoria/05_feedback_alumno.md` | Reemplaza el borrador de Fase 0 (basado solo en el PDF). Cero hallazgos críticos abiertos (H-01/H-10 cerrados y verificados). 🟡 pendiente: sincronizar al repo los cambios de texto ya reportados (F-01), subir D-04/D-07 (F-02), calibrar α contra las 5 entidades con desglose contable propio (F-03), justificar la exclusión simple del panel DAES desbalanceado (F-04). 6 mejoras 🟢 de pulido (eje de figura, supuesto de estabilidad temporal, umbral de concordancia, período declarado en alcances, notas de N, reconciliación de conteos de cooperativas). Incluye guion de preguntas esperadas de comisión. |

| Julio 2026 | Fase 6 (proyección científica) actualizada — `docs/auditoria/06_proyeccion_cientifica.md` | Completa el ciclo de 6 fases de auditoría de Ureta & Ruiz Tagle (2026). Actualizada para reflejar el cierre de H-01/H-10: se retira como pendiente la corrección del error (ya hecha) y se recalibra la Propuesta 1 (calibración de α contra las 5 CAC con desglose contable propio) como accionable *antes* de la defensa, no solo como trabajo futuro — coincide con F-03 de la Fase 5. Mantiene vigente el resto del contenido generativo: revista objetivo (*Annals of Public and Cooperative Economics*), 4 propuestas de continuidad ordenadas por dependencia lógica, vínculo con la postulación ANID ID27, y literatura no citada (verificado contra `references.bib`: Salamon, Monzón & Chaves, Antón et al. CEPAL siguen ausentes; Akerlof relativizado como sugerencia opcional, no crítica, dado que ya citan `coupet_toward_2019` para asimetrías de información). |

| 2026-07-30 | Corrección de criterio: D-04 no requiere subir el binario al repositorio | El guía determinó que basta con registrar la URL original del SII y un respaldo externo (Drive/similar) en lugar de subir `PUB_NOMBRES_PJ.txt` al repo — alojar binarios de datos grandes en git es mala práctica. Se creó `data/metadata/fuentes_externas.md` con una tabla de registro (URL + respaldo), pendiente que los autores la completen para D-04 y, cuando lo entreguen, para D-07. Se establece como convención general del Research OS, sin migrar retroactivamente los `.xlsx` ya versionados en `Dashboard_HuellaSocial` (tarea de mantenimiento futura). Actualizados `SOLICITUD_AUTORES.md`, `02_reproduccion_calculos.md`, `05_feedback_alumno.md` y `docs/reports/decision_log.md`. |
