...
2014--2024, con un total de 122 observaciones para la variable de producción total, tras excluir registros duplicados de carga de la misma cooperativa y año. El segmento CMF cubre las 7 CAC supervisadas para el período 2013--2025, con 
...
\midrule
\multirow{6}{*}{DAES} 
  & P1         & 122 & 827,8     & 392,2     & 1.119,6   & 0,0    & 10.773,7  \\
...
2~y~22~entidades por año según la disponibilidad de memorias
en el portal.
...
observaciones CAC~$\times$~año. Se define como
\textbf{observación} una combinación cooperativa-año para la
cual existe al menos el dato de producción total (P1)
disponible en el estado de resultados correspondiente,
variable que constituye el insumo mínimo indispensable para
el cálculo del VAB. Las observaciones sin datos para un
período específico no se incluyen en el panel y no se realiza
imputación ni se utilizan proxies para cubrir años faltantes.

El levantamiento inicial del segmento DAES identificó
133~filas cooperativa-año para el período 2014--2024. De
estas, 4 corresponden a cooperativas con otras variables
disponibles pero sin dato de P1 (y por tanto no constituyen
observaciones bajo la definición anterior), y 7 corresponden
a registros duplicados de carga de la misma cooperativa y año
detectados en el proceso de depuración, cuya inclusión doble
habría sobreestimado el agregado sectorial. El panel de
estimación del segmento DAES queda así conformado por
122~observaciones. El segmento CMF, con cobertura completa y
sin ambigüedades de carga, suma 91~observaciones para el
período 2013--2025.

Respecto de la vigencia legal de las cooperativas, el panel
de estimación se construye exclusivamente en función de la
disponibilidad de datos financieros y no excluye
explícitamente a entidades que hayan sido disueltas o hayan
cesado actividades con posterioridad al último período con
información disponible: una cooperativa que reportó datos
válidos en un año determinado se mantiene en el panel para
ese año independientemente de su estado legal actual. Esta
definición es consistente con el objetivo de la cuenta
satélite de medir la actividad económica efectivamente
generada por el sector en cada período, y explica además por
qué el universo de cooperativas del panel de estimación
(42, sección~\ref{sec:sintesis_trabajo}) difiere del número de cooperativas
vigentes según el registro consolidado del proyecto
(36, Figura~\ref{fig:dash_regional}).
% ============================================================
\newpage
...
\begin{itemize}
    \item Los ingresos operacionales del estado de resultados
    (línea \texttt{Total\_Ingresos\_Operación}) constituyen una
    aproximación válida de la producción total (P1) en el
    sentido del SCN 2025 (párr.~7.169) para cooperativas
    financieras de mercado.
    \item La razón CI/P es homogénea entre organizaciones
    del mismo tipo ($\alpha = 0{,}3776$). El valor de $\alpha = 0{,}3776$ corresponde a la suma de la columna de coeficientes técnicos directos de la actividad 94 (Intermediación financiera) en la Matriz Insumo-Producto 2018 del Banco Central de Chile (111 actividades), equivalente a la razón consumo intermedio/producción bruta de dicho sector." \cite{banco_central_de_chile_matriz_2018}
